...
public class SignUp {
	...
	private static boolean fileProcessing(String str, String mode) {
		...
		try {
			String path = "C:\\class\\java\\BookingMovie\\src\\BookingMovie\\dummyData\\user\\";
			...
			if(mode.equals("mkfile")) {
				...
				File newFile = new File(path+"userInfo\\"+info[0]+".txt");	//유저정보
				newFile.createNewFile();
				writer = new BufferedWriter(new FileWriter(newFile));
				writer.write("B■500\n");//브론즈 0포인트 -> 새로 생성된 아이디이므로

				newFile = new File(path+"userCoupon\\"+info[0]+".txt");	//쿠폰정보
				newFile.createNewFile();
				writer = new BufferedWriter(new FileWriter(newFile));
				writer.write("discount3");//

				newFile = new File(path+"userCard\\"+info[0]+".txt");	//카드정보
				newFile.createNewFile();
				writer = new BufferedWriter(new FileWriter(newFile));
